...
import {addbill,Viewarabica,SearchArabica} from './model/bill_schema';
...
export default function App({navigation}) {
  ...
  async function getdata(){
    setLoading(true) 
    var dar = await Viewarabica(startDate,endDate)
    setdata(dar.data)
  ...
  const filteredData = commonltr ? data.filter(x =>
    ...
    async function searchdata(query){
        ...
        var dar = await SearchArabica(startDate,endDate,query)
        setdata(dar.data)
        setweight(dar.weight)
        setep(dar.ep)
        setTotal(dar.total)
    ...
    return (
      <View style={{flexDirection:'row',width:'100%',height:'100%'}}>
        ...
        <View style={{flexDirection:'row',justifyContent:'space-between'}}>
                <Text style={{color:'#000',fontSize:20,margin:5,fontWeight:'bold'}}>Arabica Bills </Text>
                ...
                renderItem={({ item,index }) => (
                  ...
                  <View style={{justifyContent:'space-between',flexDirection:'row',width:90}}>
                      <TouchableOpacity onPress={()=>{
                          ABC.show({name:item.name,product:'Arabica',bags:parseInt(item.Weight/50).toString(),weight:item.Weight.toString(),ot:item.outern.toString(),mc:item.moisture.toString(),finalot:item.finalot.toString(),price:item.rate.toString(),total:parseInt((item.rate/50)*item.Weight).toString()},new Date(item.c_date).toLocaleString())
                      }}>
                      <Icon 
                                    name="printer" 
                                    color={'#000'}
