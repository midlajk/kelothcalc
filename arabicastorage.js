...
import {addbill,SearchArabicastorage,ViewArabicastorage} from './model/bill_schema';
...
export default function App({navigation}) {
    ...
    const [startDate, setstartDate] = useState(new Date(8,3,2000));
  ...
  async function getdata(){
    ...
    var dar = await ViewArabicastorage(startDate,endDate)
    setdata(dar.data)
    setbags(dar.bags)
    setep(dar.ep)
    setTotal(dar.data.length)
  ...
  const filteredData = commonltr ? data.filter(x =>
    ...
    async function searchrc(query){
        setLoading(true) 
        var dar = await SearchArabicastorage(startDate,endDate,query)
        setdata(dar.data)
        setbags(dar.bags)
        setep(dar.ep)
        setTotal(dar.data.length)
      ...
      <View style={{flexDirection:'row',width:'100%',height:'100%'}}>
        ...
        <View style={{height:'100%',width:'90%',padding:20}}>
            <View style={{flexDirection:'row',justifyContent:'space-between'}}>
                <Text style={{color:'#000',fontSize:20,margin:5,fontWeight:'bold'}}>Arabica Storage </Text>
            ...
            <View style={{width:250,height:100,backgroundColor:'#f1f1f1',borderRadius:16,alignItems:'center',justifyContent:'space-evenly',padding:10}}>
                <Text style={{fontWeight:'bold',fontSize:18,color:'#000'}}>Total Quantity</Text>
                <Text style={{fontWeight:'bold',fontSize:18,color:'#000'}}>{total}</Text>

                </View>
                <View style={{width:250,height:100,backgroundColor:'#f1f1f1',borderRadius:16,alignItems:'center',justifyContent:'space-evenly',padding:10}}>
                <Text style={{fontWeight:'bold',fontSize:18,color:'#000'}}>Number of Bags</Text>
                <Text style={{fontWeight:'bold',fontSize:18,color:'#000'}}>{bags}</Text>
    ...
    />
            <View style={{flexDirection:'row',justifyContent:'space-between'}}>
              ...
              <View style={{flexDirection:'row',margin:15}}>
                    <Text style={{color:'#000',fontSize:18,width:70}}>Index</Text>
                    <Text style={{color:'#000',fontSize:18,width:110}}>Date</Text>

                    <Text style={{color:'#000',fontSize:18,width:80}}>Outern</Text>

                  <Text style={{color:'#000',fontSize:18,width:80}}>Moisture</Text>
              ...
              <FlatList
                ...
                renderItem={({ item,index }) => (
                    ...
                    <View style={{flexDirection:'row',margin:15 }}>
                    <Text style={{color:'#000',fontSize:18,width:70}}>{index+1}</Text>
                <Text style={{color:'#000',fontSize:18,width:110}}>{item.c_date.toLocaleDateString()}</Text>

                    <Text style={{color:'#000',fontSize:18,width:80}}>{item.outern.toFixed(2)}</Text>
                    <Text style={{color:'#000',fontSize:18,width:80}}>{item.moisture.toFixed(1)}</Text>
