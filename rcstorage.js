...
import {addbill,SearchRcstorage,ViewRcstorage} from './model/bill_schema';
...
export default function App({navigation}) {
  ...
  async function getdata(){
    ...
    var dar = await ViewRcstorage(startDate,endDate)
    setdata(dar.data)
    setbags(dar.bags)
    setep(dar.ep)
  ...
  const filteredData = commonltr ? data.filter(x =>
    ...
    async function searchrc(query){
        setLoading(true) 
        var dar = await SearchRcstorage(startDate,endDate,query)
        setdata(dar.data)
        setbags(dar.bags)
        setep(dar.ep)
        setTotal(dar.data.length)
      ...
      <View style={{flexDirection:'row',width:'100%',height:'100%'}}>
        ...
        <View style={{height:'100%',width:'90%',padding:20}}>
            <View style={{flexDirection:'row',justifyContent:'space-between'}}>
                <Text style={{color:'#000',fontSize:20,margin:5,fontWeight:'bold'}}>Rc Storage </Text>
